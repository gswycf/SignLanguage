...
class TextTokenizer(BaseTokenizer):
    def __init__(self, tokenizer_cfg):
        ...
        if self.level == "word":
            with open(tokenizer_cfg['tokenizer_file'], 'rb') as f:
                self.token2id = pickle.load(f)  #
            self.token2id = defaultdict(lambda: self.token2id['<unk>'], self.token2id)
            self.id2token = {}
            for gls, id_ in self.token2id.items():
                self.id2token[id_] = gls

        self.tokenizer = MBartTokenizer.from_pretrained(**tokenizer_cfg)
        ...
        for single_seq in input_ids:
            ...
            for id_ in single_seq:
                if not id_ in self.pruneids:
                    ...
                    print("debug tokenizer lin85", id_, self.tokenizer.convert_ids_to_tokens(id_))
                else:
    ...
    def getids(self, input_str):
        # print("debug111==", input_str)
        input_str = [" ".join([gloss for gloss in input if gloss != "<pad>"]) for input in input_str]
        # print("debug11==", input_str)
        # input_str=[" ".join(input[:-1]) for input in input_str]
        with self.tokenizer.as_target_tokenizer():
            raw_outputs = self.tokenizer(input_str,
                                         return_attention_mask=True,
                                         return_length=True,
    ...
    def getidsforgloss(self, input_str):
        input_str = [" ".join([gloss for gloss in input if gloss != "<pad>"]) for input in input_str]
        ...
        with self.tokenizer.as_target_tokenizer():
            raw_outputs = self.tokenizer(input_str,
                                         ...
                                         padding='longest')
        pruned_input_ids = self.prune(raw_outputs['input_ids'])
        return pruned_input_ids, raw_outputs['attention_mask']

    def batch_decode(self, sequences):
        if self.level == "word":
            special_token = list(self.tokenizer.special_tokens_map.values())
            special_token, lan_token = special_token[:-1], special_token[-1]
            decoded_sequences = [' '.join([self.id2token[int(s)] for s in seq]) for seq in sequences]
            decoded_sequences = [
                " ".join(word for word in seq.split() if word not in special_token and word not in lan_token) for seq in
                decoded_sequences]
...
if __name__ == '__main__':
    ...
    # cfg['pretrained_model_name_or_path']= '../../Dataprocessing/csl-daily/'
    # cfg["pruneids_file"] = '../../Dataprocessing/csl-daily/map_ids.pkl'
    # cfg['tgt_lang']= 'zh_CN'
    # cfg['level'] = 'word'
    # cfg['tokenizer_file'] ='../../Dataprocessing/csl-daily/txt2ids.pkl'
    # # parser.set_defaults(**cfg)
    # # cfg = parser.parse_args()
    # # print(cfg)
    # tokenizers =TextTokenizer(cfg)
    # input = tokenizers(["那 将 可 以 减 少 繁 文 缛 节 .", "我 晚 上 回 到 家"])
    # print(input)
    # print(tokenizers.batch_decode(input['labels']))

    cfg['pretrained_model_name_or_path'] = '../../Dataprocessing/phoenix-2014t/'
    cfg["pruneids_file"] = '../../Dataprocessing/phoenix-2014t/map_ids.pkl'
    cfg['tgt_lang'] = 'de_DE'
    ...
    tokenizers = TextTokenizer(cfg)
    input = tokenizers(
        ["am sonntag vor allem in der südosthälfte gewitterschauer sonst setzt sich wieder meist die sonne durch .",
         'tiefausläufer erreichen uns zunächst nur in abgeschwächter form .'])
    print(input)