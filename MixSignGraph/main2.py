...
import pdb,sys
import cv2,yaml,torch, random
import importlib, faulthandler
import numpy as np
import torch.nn as nn
import shutil,inspect,time
...
def save_file(arg):
    ...
    shutil.copy2(__file__, arg.work_dir) 
def judgemetric(arg, dev_wer, best_dev):
    if arg.metric=='wer':
...
class Processor():
                ...
                if eval_model:
                    ...
                    if self.accelerator.is_main_process:
                        ...
                        self.recoder.print_log("Test WER: {:05.2f}% Test del {:05.2f}% Test ins {:05.2f}%".format(test_wer['wer'], test_wer['del'],
                                                                                        test_wer['ins']))
                        if judgemetric(self.arg, dev_wer, best_dev): #if dev_wer["rouge"] > best_dev["rouge"]: #dev_wer["wer"] < best_dev["wer"]: #
    ...
    def loading(self):
        ...
        model, optimizer.optimizer, optimizer.scheduler, self.data_loader['train'] = self.accelerator.prepare(
            model, optimizer.optimizer, optimizer.scheduler, self.data_loader['train'],
        )
    ...
    def load_model_weights(self, model, weight_path):
        ...
        # self.accelerator.load_checkpoint_in_model(weight_path)
        ...
        for name in weights:
            if name not in s_dict or "translation" in name:
