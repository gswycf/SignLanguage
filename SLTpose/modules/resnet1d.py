...
def resnet18_1d():
    net = ResNet1D(1024, 1024, kernel_size=1, stride=1, groups=1, n_block=8, n_classes=1000,
                   downsample_gap=2, increasefilter_gap=8, use_bn=True, use_do=True, verbose=False)
    return net


if __name__ == '__main__':
    net = ResNet1D(1024, 1024, kernel_size=2, stride=1, groups=1, n_block=8, n_classes=1000,
                   downsample_gap=2, increasefilter_gap=4, use_bn=True, use_do=True, verbose=False)
    x = torch.rand([1, 1024, 20])
    y = net(x)
    print(y.shape)