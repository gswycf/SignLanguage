...
def bleu(references, hypotheses, level='word'):
    ...
    :return:
    """
    # if level=='char' or is_chinese(references):
        #split word
    references = [' '.join(list(r)) if is_chinese(r) else r for r in references ]
    hypotheses = [' '.join(list(r)) if is_chinese(r) else r for r in hypotheses ]
...
def rouge(references, hypotheses, level='word'):
    ...
    ref = [list(r) if is_chinese(r) else r.split() for r in references ]
    hyp = [list(r) if is_chinese(r) else r.split() for r in hypotheses ]
...
if __name__ == '__main__':
    a =  [' in der sidosthalfte bleibt es meist trocken', 'und nun die wettervorhersage fir morgen samstag den sechsundzwanzigsten april']
    text=['im sidosten noch meist trocken', 'die aussichten am sonnta']
    print(bleu(a,text))